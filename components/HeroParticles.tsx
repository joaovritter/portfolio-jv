...
export default function HeroParticles() {
  ...
  useEffect(() => {
    ...
    const motion = 1;
    //Ajustes do hover
    const PUSH = 6; // força do empurrão do cursor (menor = mais sutil)
    const RETURN = 0.005; // rigidez da mola de volta (menor = volta mais devagar)
    const HOVER_CALM = 0.4; // agitação durante o hover (1 = normal, 0 = parado)
    const dpr = Math.min(window.devicePixelRatio || 1, 2); //escala de renderização (para não sobrecarregar)
    let W = host.clientWidth, 
      H = host.clientHeight;
    if (!W || !H) return;

    const renderer = new THREE.WebGLRenderer({ canvas, alpha: true, antialias: true }); //renderizador WebGL (motor do Three.js)
    ...
    const sample = (w: number, h: number) => { // gera pontos a partir do texto JV desenhado em um canvas 2D
    ...
    const TN = tpts.length; // quantidade de pontos que formam o "JV"
    if (!TN) {
      renderer.dispose();
      return;
    }
    // partículas soltas espalhadas pelo hero (ajuste a quantidade aqui)
    const EXTRA = isMobile ? 40 : 90;
    const N = TN + EXTRA;
    let mnX = 1e9,
      mxX = -1e9;
    for (let i = 0; i < TN; i++) {
    ...
    for (let i = 0; i < TN; i++) {
      ...
      react[i] = 0.25 + Math.random() * 0.75 ; 

      // posição inicial aleatória (animacao de entrada)
      pos[i * 3] = (Math.random() - 0.5) * W * 1.8;
      pos[i * 3 + 1] = (Math.random() - 0.5) * H * 1.8;
      pos[i * 3 + 2] = (Math.random() - 0.5) * 320;
    }

    // Partículas soltas: base aleatória por todo o hero (não fazem parte do "JV")
    for (let i = TN; i < N; i++) {
      ab[i] = Math.random(); // controla amplitude e cor (0..1)
      baseX[i] = (Math.random() - 0.5) * W * 0.95;
      baseY[i] = (Math.random() - 0.5) * H * 0.85;
      ph[i] = Math.random() * Math.PI * 2;
      react[i] = 0.25 + Math.random() * 0.75;
      pos[i * 3] = (Math.random() - 0.5) * W * 1.8;
      pos[i * 3 + 1] = (Math.random() - 0.5) * H * 1.8;
    ...
    const geo = new THREE.BufferGeometry(); // geometria de pontos (cada ponto = um pixel do texto JV)
    geo.setAttribute("position", new THREE.BufferAttribute(pos, 3));
    geo.setAttribute("color", new THREE.BufferAttribute(col, 3));
    const c1 = new THREE.Color(ACC), //
      c2 = new THREE.Color("#F4EFE6"),
      c3 = new THREE.Color("#ff0000");
    for (let i = 0; i < N; i++) { //define a cor de cada ponto com base na posição horizontal (mais próximo do centro = mais quente)
      const a = ab[i];
      const r = Math.random();
    ...
    const mat = new THREE.PointsMaterial({ // material de pontos (como partículas), podemos mudar para um material de sprite ou shader para efeitos mais avançados
    ...
    //interação com o mouse: calcula a posição do mouse em relação ao centro do canvas e aplica uma força de repulsão nos pontos próximos
    ...
    const tick = () => { 
      ...
      for (let i = 0; i < N; i++) {
        const ix = i * 3,
          iy = i * 3 + 1,
          iz = i * 3 + 2;
        const amp = (8 + ab[i] * 24) * motion * calm;
        const ox =
    ...
    const onResize = () => {
      ...
      const ts = sample(W, H);
      for (let i = 0; i < TN && i < ts.length; i++) {
        baseX[i] = ts[i][0];
        baseY[i] = ts[i][1];
      }
    };
